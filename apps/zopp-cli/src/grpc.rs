...
mod tests {
    ...
    fn test_add_auth_metadata_success() {
        ...
        let result = add_auth_metadata(&mut request, &principal, method);
        assert!(result.is_ok());
    ...
    #[test]
    fn test_add_auth_metadata_invalid_key() {
        use crate::config::PrincipalConfig;

        let principal = PrincipalConfig {
            id: "test-principal-id".to_string(),
            name: "test-principal".to_string(),
            private_key: "invalid_key".to_string(), // Invalid
            public_key: "1".repeat(64),
