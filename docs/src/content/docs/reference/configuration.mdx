...
This allows you to:
- Keep one config at the project root
- Run zopp commands from any subdirectory
- Have different configs for different projects
...
├── services/
│   ├── api/
│   │   └── zopp.toml      # project = "api"
│   ├── web/
│   │   └── zopp.toml      # project = "web"
│   └── worker/
│       └── zopp.toml      # project = "worker"
└── zopp.toml              # workspace = "mycompany" (shared)
```

Each service can have its own config that inherits from the parent:

```toml
# services/api/zopp.toml
[defaults]
workspace = "mycompany"
project = "api"
environment = "development"
...
```bash
# .gitignore
# Don't ignore zopp.toml - it's safe to commit
```
...
```toml
# zopp.toml
# Secrets manager configuration for the API backend service
# See: https://docs.zopp.dev/reference/configuration/

[defaults]
workspace = "mycompany"      # Our organization's workspace
project = "api-backend"       # This service
environment = "development"   # Safe default
```

## Validation

zopp validates the configuration file on load:

```bash
# Invalid TOML syntax
zopp secret list
# Error: Failed to parse zopp.toml: expected '=' at line 2

# Unknown key
zopp secret list
# Warning: Unknown key 'workspce' in zopp.toml (did you mean 'workspace'?)
```

<Aside type="tip">
  If you're having issues, try running with `--verbose` to see which config file is being used.
</Aside>
